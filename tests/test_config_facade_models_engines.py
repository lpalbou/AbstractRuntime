"""Models & engines passthroughs in `config_facade` (AbstractCore >= 2.15.3).
...
def test_new_surface_is_exported_and_callable() -> None:
    ...
    assert facade.MODELS_ENGINES_MIN_ABSTRACTCORE == "2.15.3"
...
def test_an_older_abstractcore_raises_too_old_with_the_upgrade_command(monkeypatch, module: str, call) -> None:
    ...
    assert err.required == "2.15.3"
    assert err.missing == module
    assert "abstractcore>=2.15.3" in str(err) and "2.13.42" in str(err)
