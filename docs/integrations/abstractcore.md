  ...
  "tool_calls": [
    {
      "name": "tool_name",
      "arguments": {"x": 1},
      "call_id": "optional (model/provider id)",
...
Notes:
- `call_id` is often emitted by the model/provider and may change across retries/restarts; it is treated as **non-semantic** for idempotency.
- `runtime_call_id` is generated by AbstractRuntime and is stable for the same logical tool call; remote workers should use it as their idempotency key.
