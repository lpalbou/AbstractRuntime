...
def test_runtime_base_is_remote_light_with_multimodal_and_mcp_support() -> None:
    ...
    assert '"abstractcore[remote,tools,vision,voice,audio,music]>=2.16.0"' in text
...
def test_runtime_exposes_only_apple_and_gpu_user_install_profiles() -> None:
    ...
    assert '"abstractcore[all-apple]>=2.16.0"' in apple_block
    assert '"abstractcore[all-gpu]>=2.16.0"' in gpu_block
